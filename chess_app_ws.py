...
# =========================
# FILE: chess_app_ws.py  (기존 ChessApp 통합본 예시)
# DESC: HTTP POST 제거, WebSocket 연동(ESP32WS)으로 변경
# NOTE: 프로젝트 구조에 맞게 파일명/모듈 경로 조정하세요.
# =========================
...
from esp32_ws import ESP32WS   # <<< 추가: WS 연동
...
class ChessApp(tk.Tk):
    def __init__(self):
        ...
        # ---- ESP32 WebSocket 연결 ----
        self.ws = ESP32WS("ws://10.91.224.3/ws")  # IP/경로 맞게 수정
        # 초기 보드 동기화가 필요하면(선택):
        self.ws.send_board(tiles=8, square=8)
    ...
    def _build_menu(self):
        ...
        game.add_separator()
        game.add_command(label="Exit", command=self._on_close)
    ...
    def _load_fen(self):
        ...
        try:
            ...
            self.status_var.set(self.game.status_text())
            self.board_view.redraw()
            # 로드 후 ESP 보드도 맞추고 싶다면 reset/board 호출(선택)
            try:
                self.ws.reset()
                self.ws.send_board(tiles=8, square=8)
            except Exception as e:
